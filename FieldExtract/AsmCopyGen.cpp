    ...
    switch (size) {
        case 1: {
            ...
            if (target == 0) {
                ...
                insert(
                    {0x88, 0x86, uint8_t(target), uint8_t(target >> 8), uint8_t(target >> 16), uint8_t(target >> 24)});
            }
        }; break;
        case 2: {
        ...
        case 4: {
            ...
            if (source == 0) {
                ...
                insert(
                    {0x8b, 0x87, uint8_t(source), uint8_t(source >> 8), uint8_t(source >> 16), uint8_t(source >> 24)});
            }
            // mov %eax,target(%rsi)
            if (target == 0) {
                insert({0x89, 0x06});
                ...
                insert(
                    {0x89, 0x86, uint8_t(target), uint8_t(target >> 8), uint8_t(target >> 16), uint8_t(target >> 24)});
            }
        }; break;
        case 8: {
            // mov source(%rdi), %rax
